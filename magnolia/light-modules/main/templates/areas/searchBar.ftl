[#if content.searchPage?has_content]
    <form action="${cmsfn.link('website', content.searchPage)!}" class="form o-flex-inline o-search">
        <label class="search-label is-visually-hidden" for="searchInput">${content.label!}</label>
        <input id="searchInput" type="search" name="q" placeholder="${content.placeholder!}" class="search-input" [#if !queryStr?has_content]autofocus[#else]value="${queryStr}"[/#if]>
