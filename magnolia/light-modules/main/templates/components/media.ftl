...
[#assign isCover = (content.isCover?? && content.isCover == true) || hasRatio]
...
<!-- Media -->
<section class="o-section [#if hasBackground]has-background -${content.backgroundTheme!} is-full-width[/#if] [#if hasInnerSpace]has-inner-space[/#if] [#if hasNoTopSpace]has-no-top-space[/#if] [#if hasNoBottomSpace]has-no-bottom-space[/#if] [#if isFullWidth]is-full-width[/#if]">
    <div class="o-group [#if isFullWidth]is-full-width[#elseif isLarge]is-large[/#if]">
            ...
            [#if content.video?has_content]
                ...
                <div>
                    ...
                    <template v-if="source">
                        <video autoplay loop muted playsinline poster="${damfn.getAssetLink(content.image)!}" v-bind:class="{ 'js-loaded': source }" class="container [#if hasRatio]has-fixed-ratio[/#if] media is-${content.position!"center"} [#if isCover]is-cover[/#if]">
        ...
        [#else]
            <picture class="container js-loaded [#if hasRatio]has-fixed-ratio[/#if]">
                <img class="media [#if isCover]is-cover[/#if]" src="${damfn.getAssetLink(content.image)!}" style="display:block;max-width:100%">
            </picture>
        [/#if]
        </div>
    </div>
