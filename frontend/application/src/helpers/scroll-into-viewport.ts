...
export default function scrollIntoViewport(
     ...
     */
    return async function(element: HTMLElement): Promise<boolean> {
     ...
     */
    async function scrollTo(goTo: number): Promise<any> {
 ...
 */
function getTopPosition(element: any, boundary = window) {
    let top = element.offsetTop;

    while (
        (element = /** @type {HTMLElement} */ (element.offsetParent)) !== null &&
        element !== boundary
        ) {
...
function bezierEasing(bezierArray: number[]) {
    /**
    ...
    return (function bezier([mX1, mY1, mX2, mY2] = bezierArray) {
